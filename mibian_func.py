...
def impliedVolatility(className, args, callPrice=None, putPrice=None, high=500, low=0.0):
	'''Returns the estimated implied volatility'''
	if callPrice:
		target = callPrice
	if putPrice:
		target = putPrice
	decimals = 4		# Count decimals
...
class BS:
	...
	def __init__(self, args, volatility=None, callPrice=None, putPrice=None, \
		...
		self.daysToExpiration = float(args[3]) / 252
